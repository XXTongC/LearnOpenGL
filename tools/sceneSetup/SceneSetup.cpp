...
namespace
{
	...
	void prepareScreenPass(GL_SCENE::SetupContext& context)
	{
		...
		context.screenMaterial->setInputTextures(
			context.frameRenderTargets.getResolvedColorAttachment(),
			context.frameRenderTargets.getBloomPongColorAttachment(),
			context.frameRenderTargets.getResolvedDepthStencilAttachment()
		);
