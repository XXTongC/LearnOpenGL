...
- 当前剩余明显问题：C 盘空间仍偏低，完整 MSBuild / runtime smoke 需要继续关注输出体积；PBR IBL 已有自动化 scene/capture 验证但还没有人工视觉审阅；工程内仍没有默认真实 HDR environment 资源；PBR deferred path 已有 G-buffer producer / debug consumer / lighting consumer、directional / point atlas shadow sampling、SSBO light buffer 和 shadow atlas 写入链路，但透明 forward fallback、material feature parity 和 clustered light culling / tile index list 还未实现。
- 下一步建议目标：开始处理 PBR deferred path 的透明 forward fallback，或者继续把 light buffer 演进为 clustered / tiled light index list。
