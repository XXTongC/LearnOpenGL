...
- 当前 renderer 内部 pass 顺序已收敛到 `RendererFramePassRegistry`，并已支持通过 pass order string 构建执行计划；后续 PBR shadow atlas / IBL debug / G-buffer pass 可以继续按 key 增加和插入，而不必扩写 `Renderer::render()` 主流程。
