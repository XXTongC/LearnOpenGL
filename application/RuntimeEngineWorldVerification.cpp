...
namespace GL_RUNTIME
{
	...
	{
		const auto& engineWorld = verification.engineWorld;
		const auto renderStats = RuntimeEngineWorldVerificationResourceAdapter::collectPreparedSceneStats(
			context.renderResources
		);
		EngineWorldPreparedSceneStats stats{};
		stats.engineWorldProbeMeshCount = renderStats.engineWorldProbeMeshCount;
		stats.engineWorldMinimalMeshCount = renderStats.engineWorldMinimalMeshCount;
		...
		if (context.engineAttachments.engineWorld && context.engineAttachments.engineWorldEditable)
		{
			...
			if (engineWorld.enableScenePackageRoundTrip)
			{
				...
				const auto packageLoad =
					RuntimeEngineWorldVerificationResourceAdapter::loadRuntimeGeneratedScenePackage(
						context.renderResources,
						packagePath
					);
