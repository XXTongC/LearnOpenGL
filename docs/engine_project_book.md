...
当前长期方向：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 Bloom Header Framebuffer Boundary Cleanup 后，下一步优先继续 runtime/renderer header surface audit 或 Engine public header 的低风险 include audit；当前不建议继续扩张 PBR 功能。

当前最新修正：Bloom Header Framebuffer Boundary Cleanup 已接入后，`Bloom.h` 只保留 `Framebuffer` / `Texture` / `Shader` / `Geometry` 的 forward declarations，完整 Bloom FBO/shader/geometry 执行依赖下沉到 `Bloom.cpp`；下一步继续 runtime/renderer header surface audit 或 Engine public header 的低风险 include audit；当前不建议继续扩张 PBR 功能。
