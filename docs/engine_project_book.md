...
当前长期方向：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 PBR Shadow Resource Binder Header Boundary Cleanup 后，下一步优先继续 runtime/renderer header surface audit 或 Engine public header 的低风险 include audit；当前不建议继续扩张 PBR 功能。

当前最新修正：PBR Shadow Resource Binder Header Boundary Cleanup 已接入后，`PBRShadowResourceBinder.h` 只保留 shader 和 material binding context 参数类型前置声明，完整 CSM/atlas shadow 资源绑定依赖下沉到 `PBRShadowResourceBinder.cpp`；下一步继续 runtime/renderer header surface audit 或 Engine public header 的低风险 include audit；当前不建议继续扩张 PBR 功能。
