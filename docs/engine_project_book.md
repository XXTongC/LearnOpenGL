...
当前长期方向：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 Application Header Boundary Cleanup 后，下一步优先继续 application composition root / runtime context state 依赖边界收敛，或继续 legacy/runtime public header 的低风险 include audit；当前不建议继续扩张 PBR 功能。

当前最新修正：Application Header Boundary Cleanup 已接入后，`Application.h` 不再传播 texture/logger implementation headers 或未使用的 global logger 声明；下一步继续 application composition root / runtime context state 依赖边界收敛，但不扩张 PBR pass。
