...
当前长期方向：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 Runtime Render Resource ReadOnly View Facade Removal 后，下一步应重新审计剩余 direct render resource accessor 是否已经全部位于 resource adapter implementation；如果是，render resource decoupling 可阶段性收束，转入更高层 Engine runtime ownership 或 editor/gameplay boundary，当前不建议继续扩张 PBR 功能。
