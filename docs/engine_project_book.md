...
当前长期方向：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 Assimp Instance Loader GLM Header Boundary Cleanup 后，下一步优先继续 application composition root / runtime context state 依赖边界收敛，或继续 legacy/runtime public header 的低风险 include audit；当前不建议继续扩张 PBR 功能。

当前最新修正：Assimp Instance Loader GLM Header Boundary Cleanup 已接入后，`assimpInstanceLoader.h` 不再传播完整 `glm.hpp`，矩阵完整定义和 instanced matrix 写入依赖局部化到 implementation；下一步继续 application composition root / runtime context state 依赖边界收敛，但不扩张 PBR pass。
