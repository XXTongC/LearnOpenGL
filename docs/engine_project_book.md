...
当前长期方向：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 Renderer Backend Contract Frame DTO Header Boundary Cleanup 后，下一步优先继续通用 renderer backend contract/header surface audit 或 Engine public header 的低风险 include audit；当前不建议继续扩张 PBR 功能。

当前最新修正：Renderer Backend Contract Frame DTO Header Boundary Cleanup 已接入后，`RendererBackend.h` 只暴露 backend contract 和 DTO 前置声明，完整 frame DTO 依赖下沉到 concrete runtime backend implementation；下一步继续通用 renderer backend contract/header surface audit 或 Engine public header 的低风险 include audit；当前不建议继续扩张 PBR 功能。
