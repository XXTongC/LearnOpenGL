...
当前长期方向：Engine runtime ownership 已完成多轮 boundary/header extraction，render resource decoupling 已阶段性收束。本轮 Runtime Frame Pipeline Profile Config Schema Adapter 后，profile/settings 类中直接暴露 `visitEditableProperties(PropertyBuilder&)` 的边界已清完；下一步应继续推进系统化 UI/inspector 与 editor/gameplay boundary，建议检查 config/provider 层的 `PropertyBuilder` 依赖是否都处在合理边界，或继续拆 DebugControllerPanel 的 profile panel 编排；当前不建议继续扩张 PBR 功能。
