...
当前长期方向：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 Runtime Frame Pass Registry Profile Predicate Cleanup 后，确认剩余 direct render resource accessor 已集中在 resource adapter/service implementation，render resource decoupling 可阶段性收束；下一步应转入更高层 Engine runtime ownership、editor/gameplay boundary、场景/资产生命周期或系统化 UI/inspector 边界整理，当前不建议继续扩张 PBR 功能。
