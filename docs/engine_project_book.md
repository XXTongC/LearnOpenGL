...
当前长期方向：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 PBR Surface Resource Binder Header Boundary Cleanup 后，下一步优先继续 runtime/renderer header surface audit 或 Engine public header 的低风险 include audit；当前不建议继续扩张 PBR 功能。

当前最新修正：PBR Surface Resource Binder Header Boundary Cleanup 已接入后，`PBRSurfaceResourceBinder.h` 只保留 shader 和 PBR material 参数类型前置声明，完整 surface uniform 与 texture binding 依赖下沉到 `PBRSurfaceResourceBinder.cpp`；下一步继续 runtime/renderer header surface audit 或 Engine public header 的低风险 include audit；当前不建议继续扩张 PBR 功能。
