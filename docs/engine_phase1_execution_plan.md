...
当前最新修正：Runtime Probe Scene Resource Adapter Cleanup 已接入后，verification probe 对 offscreen scene readiness、PBR probe geometry 创建和 probe object 注入的访问已集中到 `RuntimeProbeSceneResourceAdapter`，`RuntimePBRSceneProbeVerification.cpp` 与 `RuntimeImportedAssetVerification.cpp` 不再直接调用 render resource probe helper，`RuntimeRenderResourceState` 删除 probe-only public helper。

下一阶段建议继续推进 Engine runtime ownership：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 probe scene resource adapter cleanup 后，下一步应继续审计非 adapter 调用方中剩余的窄 render resource state 方法，例如 frame runner clear color sync 与 renderer frame pass profile access；如果这些也能收束到 adapter，则 render resource decoupling 可阶段性收束，转入更高层 Engine runtime ownership 或 editor/gameplay boundary；当前不建议继续扩张 PBR pass。
