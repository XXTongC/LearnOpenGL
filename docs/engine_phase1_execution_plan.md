...
当前最新修正：Renderer Infrastructure and Runtime Input Header Boundary Cleanup 已接入后，`RenderQueue.h`、`ShadowRenderer.h`、`ShaderLibrary.h`、`PBRShadowAtlasRenderTargets.h` 与 `RuntimeInputController.h` 不再传播 camera/scene/mesh/shader/core/camera-control 等 implementation-only headers；完整 render queue、shadow renderer、shader construction、shadow atlas GL texture 和 input controller 依赖下沉到对应 `.cpp`；下一步继续 runtime/renderer header surface audit，优先审计宽 facade，但不扩张 PBR pass。

下一阶段建议继续推进 Engine runtime ownership：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 renderer infrastructure/runtime input header cleanup 后，下一步优先继续 runtime/renderer header surface audit 或 Engine public header 低风险 include audit；当前不建议继续扩张 PBR pass。
