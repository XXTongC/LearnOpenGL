...
当前最新修正：Runtime Render Resource ReadOnly View Facade Removal 已接入后，`RuntimeFrameReadinessResourceAdapter.cpp` 改为直接使用 `RuntimeRenderResourceState` const accessor，`RuntimeRenderResourceState` 删除 `RuntimeRenderResourceView` class 与 `readOnlyView()` public facade，application 源码不再存在 read-only view 过渡 API。

下一阶段建议继续推进 Engine runtime ownership：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 read-only view facade removal 后，下一步应重新审计剩余 direct render resource accessor 是否已经全部位于 resource adapter implementation；如果是，render resource decoupling 可阶段性收束，转入更高层 Engine runtime ownership 或 editor/gameplay boundary；当前不建议继续扩张 PBR pass。
