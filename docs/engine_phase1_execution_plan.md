...
当前最新修正：IBL Precompute Pass Header Boundary Cleanup 已接入后，`IBLPrecomputePass.h` 只保留 environment targets、texture、mesh、shader、shader library 前置声明，完整 capture-view/texture/shader/library 依赖下沉到 `IBLPrecomputePass.cpp`；下一步继续 runtime/renderer header surface audit 或 Engine public header 低风险 include audit；当前不建议继续扩张 PBR pass。

下一阶段建议继续推进 Engine runtime ownership：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 IBL precompute pass header boundary cleanup 后，下一步优先继续 runtime/renderer header surface audit 或 Engine public header 低风险 include audit；当前不建议继续扩张 PBR pass。
