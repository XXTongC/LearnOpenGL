...
当前最新修正：Runtime Render Resource Renderer/Scene Owner Boundary Cleanup 已接入后，`RuntimeRenderResourceState.h` 已移除 `renderer` / `sceneOffScreen` / `sceneInScreen` 公开 owner 字段，改由 private 成员和 accessor 暴露当前必要写入/读取契约；不改变 renderer backend、scene setup、frame pass 或 PBR pass。

下一阶段建议继续推进 Engine runtime ownership：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 renderer/scene owner cleanup 后，`RuntimeRenderResourceState` 剩余显著公开状态主要是值型 `clearColor`；下一步可做低风险 accessor 收敛或设计 read-only runtime resource facade，当前不建议继续扩张 PBR pass。
