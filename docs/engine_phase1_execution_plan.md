...
当前最新修正：Application Header Boundary Cleanup 已接入后，`Application.h` 不再传播 texture/logger implementation headers 或未使用的 global logger 声明；下一步继续 application composition root / runtime context state 依赖边界收敛，或继续 legacy/runtime public header 低风险 include audit，但不扩张 PBR pass。

下一阶段建议继续推进 Engine runtime ownership：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 application header boundary cleanup 后，下一步优先继续 application composition root / runtime context state 依赖边界收敛，或继续 legacy/runtime public header 低风险 include audit；当前不建议继续扩张 PBR pass。
