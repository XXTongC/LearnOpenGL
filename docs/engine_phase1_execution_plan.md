...
当前最新修正：Light Resource Binder Header Boundary Cleanup 已接入后，`LightResourceBinder.h` 只保留 shader/light 参数类型前置声明，完整 shader uniform 写入和 light 字段读取依赖下沉到 `LightResourceBinder.cpp`；下一步继续 runtime/renderer header surface audit 或 Engine public header 低风险 include audit；当前不建议继续扩张 PBR pass。

下一阶段建议继续推进 Engine runtime ownership：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 light resource binder header boundary cleanup 后，下一步优先继续 runtime/renderer header surface audit 或 Engine public header 低风险 include audit；当前不建议继续扩张 PBR pass。
