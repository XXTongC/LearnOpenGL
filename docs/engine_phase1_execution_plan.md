...
下一阶段建议继续推进 Engine runtime ownership：Engine runtime ownership 已完成多轮 boundary/header extraction。本轮 read-only view facade cleanup 后，下一步可继续把更多只读 consumer 迁到 `RuntimeRenderResourceView`，或转向其他 runtime/application state 的依赖边界；当前不建议继续扩张 PBR pass。
