...
### 2026-05-31 Light Resource Binder Header Boundary Cleanup

本轮继续 runtime/renderer header surface audit。审计确认：`LightResourceBinder.h` 为了一个 forward-light uniform 绑定入口直接 include 完整 `framework/shader.h`、`ambientLight.h`、`directionalLight.h`、`pointLight.h` 和 `spotLight.h`，但 header 只需要声明 `std::shared_ptr` 参数类型；真正写 shader uniform、读取 light 字段和查询 `PointLightShadow::getMAX_POINT_LIGHT()` 的逻辑全部在 implementation 内。

新增与修改：

- `LightResourceBinder.h` 移除完整 shader/light implementation includes。
- `LightResourceBinder.h` 新增 `Shader`、`DirectionalLight`、`SpotLight`、`PointLight` 和 `AmbientLight` forward declarations，保留现有 `bindForwardLights(...)` public API。
- `LightResourceBinder.cpp` 显式 include `framework/shader.h` 与 light implementation headers，确保 uniform 写入和 light 字段读取依赖位于实际实现文件。

已完成验证：

- 静态检查确认 `LightResourceBinder.h` 不再 include 完整 shader/light headers，`LightResourceBinder.cpp` 显式 include 所需完整依赖。
- 静态检查确认调用点仍集中在 `MaterialBinder.cpp` 与 `PBRMaterialBinder.cpp`。
- `powershell -NoProfile -ExecutionPolicy Bypass -File tools\verify_pbr.ps1 -NoLinkDebugInfo -Modes forward,renderer-backend-registry-noop,engine-world-minimal-scene,engine-world-scene-package -DiscardCaptures`：构建通过；MSBuild 明确编译 `LightResourceBinder.cpp`、`MaterialBinder.cpp` 与 `PBRMaterialBinder.cpp`；四条 focused verification mode 全部通过。
- `powershell -NoProfile -ExecutionPolicy Bypass -File tools\verify_pbr.ps1 -SkipBuild -DiscardCaptures`：默认 34 个 verification mode 全部通过。
...
### 2026-06-01 Material Binder Header Boundary Cleanup

本轮继续 runtime/renderer header surface audit。审计确认：`MaterialBinder.h` 只声明 legacy/Phong/PBR material binding facade，却为了一个 `bind(...)` 函数签名直接 include 完整 `framework/shader.h`、`materials/material.h`、`mesh/mesh.h` 和 `MaterialBindingContext.h`。实际材质类型分发、texture binding、shader uniform 写入、mesh model matrix 读取和 PBR binder 转发全部在 `MaterialBinder.cpp` 中，header 可以只保留参数类型前置声明。

新增与修改：

- `MaterialBinder.h` 移除完整 shader、material、mesh 和 material binding context includes。
- `MaterialBinder.h` 新增 `Shader`、`Material`、`Mesh` 和 `MaterialBindingContext` forward declarations，保留现有 `bind(...)` public API。
- `MaterialBinder.cpp` 显式 include `framework/shader.h`、`framework/texture.h`、`materials/material.h`、`mesh/mesh.h` 和 `renderer/MaterialBindingContext.h`，因为 implementation 实际读取材质、贴图、shader、mesh 和绑定上下文字段。

已完成验证：

- 静态检查确认 `MaterialBinder.h` 不再 include 完整 shader/material/mesh/material binding context headers，`MaterialBinder.cpp` 显式 include 所需完整依赖。
- 静态检查确认调用点集中在 `SceneRenderPass.cpp`，MaterialBinder public API 未改名也未改变参数。
- `powershell -NoProfile -ExecutionPolicy Bypass -File tools\verify_pbr.ps1 -NoLinkDebugInfo -Modes forward,renderer-backend-registry-noop,engine-world-minimal-scene,engine-world-scene-package -DiscardCaptures`：构建通过；MSBuild 明确编译 `MaterialBinder.cpp` 与 `SceneRenderPass.cpp`；四条 focused verification mode 全部通过。
- `powershell -NoProfile -ExecutionPolicy Bypass -File tools\verify_pbr.ps1 -SkipBuild -DiscardCaptures`：默认 34 个 verification mode 全部通过。

结论：

- 这是 MaterialBinder public include boundary cleanup，不改变 legacy/Phong/PBR material binding 分发逻辑、texture unit 绑定、shader uniform layout、runtime frame pipeline frame plan、renderer backend registry/no-op verification、Engine World verification 或 PBR pass。
- 后续建议继续 runtime/renderer header surface audit，或回到 Engine public header 的低风险 implementation detail audit；当前仍不建议继续扩张 PBR 功能。

### 2026-06-01 PBR Material Binder Header Boundary Cleanup

本轮继续 runtime/renderer header surface audit。审计确认：`PBRMaterialBinder.h` 只声明 PBR material binding facade，却为了一个 `bind(...)` 函数签名直接 include 完整 `framework/shader.h`、`PBRMaterial.h`、`mesh/mesh.h` 和 `MaterialBindingContext.h`。真正的 PBR object uniforms、forward lights、shadow resources、surface textures 与 IBL binding 编排全部在 `PBRMaterialBinder.cpp` 中，header 可以只保留参数类型前置声明。

新增与修改：

- `PBRMaterialBinder.h` 移除完整 shader、PBR material、mesh 和 material binding context includes。
- `PBRMaterialBinder.h` 新增 `Shader`、`PBRMaterial`、`Mesh` 和 `MaterialBindingContext` forward declarations，保留现有 `bind(...)` public API。
- `PBRMaterialBinder.cpp` 显式 include `framework/shader.h`、`materials/pbrMaterial/PBRMaterial.h`、`mesh/mesh.h` 和 `renderer/MaterialBindingContext.h`，因为 implementation 实际校验 binding context 并转发到 PBR object/light/shadow/surface/IBL binders。

已完成验证：

- 静态检查确认 `PBRMaterialBinder.h` 不再 include 完整 shader/PBR material/mesh/material binding context headers，`PBRMaterialBinder.cpp` 显式 include 所需完整依赖。
- 静态检查确认调用点集中在 `MaterialBinder.cpp` 与 `PBRSceneRenderPass.cpp`，PBRMaterialBinder public API 未改名也未改变参数。
- `powershell -NoProfile -ExecutionPolicy Bypass -File tools\verify_pbr.ps1 -NoLinkDebugInfo -Modes forward,renderer-backend-registry-noop,engine-world-minimal-scene,engine-world-scene-package -DiscardCaptures`：构建通过；MSBuild 明确编译 `MaterialBinder.cpp`、`PBRMaterialBinder.cpp` 与 `PBRSceneRenderPass.cpp`；四条 focused verification mode 全部通过。
- `powershell -NoProfile -ExecutionPolicy Bypass -File tools\verify_pbr.ps1 -SkipBuild -DiscardCaptures`：默认 34 个 verification mode 全部通过。
...
### 2026-06-01 PBR Shadow Resource Binder Header Boundary Cleanup

本轮继续 runtime/renderer header surface audit。审计确认：`PBRShadowResourceBinder.h` 只声明 PBR shadow resource binding facade 和轻量结果 DTO，却为了 `bind(...)` / `bindDetailed(...)` / `getCsmLayerCount(...)` 参数签名直接 include 完整 `framework/shader.h` 和 `MaterialBindingContext.h`。真正读取 camera、directional/point light shadow、PBR shadow atlas render targets、legacy CSM fallback 并写 shader uniform / texture binding 的逻辑全部在 `PBRShadowResourceBinder.cpp` 中，header 可以只保留参数类型前置声明。

新增与修改：

- `PBRShadowResourceBinder.h` 移除完整 shader 和 material binding context includes。
- `PBRShadowResourceBinder.h` 新增 `Shader` 与 `MaterialBindingContext` forward declarations，保留现有 `bind(...)`、`bindDetailed(...)`、`getCsmLayerCount(...)` public API 和 result DTO。
- `PBRShadowResourceBinder.cpp` 显式 include `camera/camera.h`、`framework/shader.h` 和 `renderer/MaterialBindingContext.h`，因为 implementation 实际读取 camera/context 字段并写 shader uniform；shadow/light/atlas 依赖继续保留在 implementation。

已完成验证：

- 静态检查确认 `PBRShadowResourceBinder.h` 不再 include 完整 shader/material binding context headers，`PBRShadowResourceBinder.cpp` 显式 include 所需完整依赖。
- 静态检查确认调用点集中在 `PBRMaterialBinder.cpp` 与 `PBRDeferredLightingPass.cpp`，PBRShadowResourceBinder public API 未改名也未改变参数。
- `powershell -NoProfile -ExecutionPolicy Bypass -File tools\verify_pbr.ps1 -NoLinkDebugInfo -Modes forward,deferred,renderer-backend-registry-noop,engine-world-minimal-scene,engine-world-scene-package -DiscardCaptures`：构建通过；MSBuild 明确编译 `PBRShadowResourceBinder.cpp`、`PBRMaterialBinder.cpp` 与 `PBRDeferredLightingPass.cpp`；五条 focused verification mode 全部通过。
- `powershell -NoProfile -ExecutionPolicy Bypass -File tools\verify_pbr.ps1 -SkipBuild -DiscardCaptures`：默认 34 个 verification mode 全部通过。
...
### 2026-06-01 Runtime Profile State Light Rig Owner Boundary Cleanup

本轮继续 application runtime profile/state public header include surface 收敛，不扩张 PBR 功能。审计确认：`RuntimeProfileState.h` 仍为了按值持有 `PBRLightRigProfile` 直接 include `PBRLightRigProfile.h`；但 light rig profile 的真实读写集中在 PBR light/camera verification、profile loader、scene setup context 和 editor/debug controller context 构造，访问面虽然大于 camera/environment，但仍可通过访问器局部化完整依赖。

新增与修改：

- `RuntimeProfileState.h` 移除 `PBRLightRigProfile.h` include，改为 forward declare `GL_SCENE::PBRLightRigProfile`。
- `RuntimeProfileState` 改为通过 private `std::unique_ptr<GL_SCENE::PBRLightRigProfile>` 持有 light rig profile，继续禁用拷贝并保留移动语义。
- `RuntimeProfileState` 新增 `pbrLightRigProfile()` / `pbrLightRigProfile() const` 访问器。
- `RuntimeProfileState.cpp` 显式 include `PBRLightRigProfile.h`，集中创建、销毁和访问 light rig profile owner。
- `RuntimePBRLightCameraRigVerification.cpp` 在真实写入 verification light preset 的 implementation 中显式 include `PBRLightRigProfile.h`，并通过局部引用写入 ambient、directional、spot 和 point light preset。
- `RuntimeProfileLoader.cpp`、`RuntimeSceneSetupContextFactory.cpp` 与 `RuntimeEditorPanelCoordinator.cpp` 改为通过访问器向 PBR experiment profile loader、scene setup 和 debug controller context 传递 light rig profile 引用或指针。

已完成验证：

- 静态检查确认 `RuntimeProfileState.h` 不再传播 `PBRLightRigProfile.h`，旧字段式 `context.profiles.pbrLightRigProfile` 访问已迁移为访问器调用。
- `git diff --check` 已通过；仅报告现有 LF/CRLF 工作区提示，无 whitespace error。
- focused verification：`powershell -NoProfile -ExecutionPolicy Bypass -File tools\verify_pbr.ps1 -NoLinkDebugInfo -Modes forward,deferred,deferred-tiled-lights,showcase-spheres,engine-world-minimal-scene,renderer-backend-registry-noop -DiscardCaptures` 已通过；MSBuild 明确重新编译 `RuntimeProfileState.cpp`、`RuntimePBRLightCameraRigVerification.cpp`、`RuntimeProfileLoader.cpp`、`RuntimeSceneSetupContextFactory.cpp`、`RuntimeEditorPanelCoordinator.cpp` 和相关 runtime 使用点。
- full verification：`powershell -NoProfile -ExecutionPolicy Bypass -File tools\verify_pbr.ps1 -SkipBuild -DiscardCaptures` 已通过，默认 34 个 verification mode 全部通过。
...
### 2026-06-01 Runtime Render Resource Bloom Owner Boundary Cleanup

本轮转向 remaining runtime render resource owner/shared_ptr audit，不扩张 PBR 功能。审计确认：`RuntimeRenderResourceState.h` 虽然已经不传播 `Bloom.h`，但仍把 `std::shared_ptr<GLframework::Bloom>` 作为公开可变字段暴露；真实访问集中在 scene setup 注入、Bloom frame pass 执行和 renderer backend readiness gate，适合作为小切片收敛。

新增与修改：

- `RuntimeRenderResourceState.h` 移除公开字段 `bloom`，改为 private `mBloom`。
- `RuntimeRenderResourceState` 新增 `bloom()` / `bloom() const` 访问器，返回 shared pointer 引用以保持 scene setup assignment 契约。
- `RuntimeRenderResourceState.cpp` 集中提供 Bloom owner 访问器 implementation。
- `RuntimeSceneSetupContextFactory.cpp` 通过访问器把 Bloom owner 引用传入 `SetupContext`，保持 `SceneSetup.cpp` 初始化 Bloom 的原行为。
- `RuntimeFramePasses.cpp` 和 `RuntimeRendererFrameBridgeAdapter.cpp` 改为通过访问器执行 Bloom bright extraction、blur 和 readiness 检查。

已完成验证：

- 静态检查确认旧字段式 `context.renderResources.bloom` 访问已清零，剩余访问均为 `bloom()` accessor。
- `git diff --check` 已通过；仅报告现有 LF/CRLF 工作区提示，无 whitespace error。
...
### 2026-06-01 Runtime Selection Inspector Provider Registry

本轮开始推进类型/组件 property provider 注册机制的第一步。此前 selection inspector 已经拆到独立 implementation，但 `drawSelectionInspectorPanel(...)` 仍直接手写 Asset / Component / Actor / Shadow / Camera / Object 的 if/return 分发。这样每新增一种可检查目标，都需要继续修改主 draw 函数。本轮引入 provider registry，让主面板只负责创建 provider context 和调用匹配 provider。

新增与修改：

- 新增 `tools/inspector/SelectionInspectorProviderRegistry.h/.cpp`，提供 `SelectionInspectorProviderContext`、`SelectionInspectorProvider` 和 `SelectionInspectorProviderRegistry`，支持注册 provider、按顺序匹配 provider、绘制第一个匹配 provider。
- `SelectionInspectorPanel.cpp` 为 Asset、Component、Actor、Shadow、Camera、Object 注册默认 provider。
- `drawSelectionInspectorPanel(...)` 删除顶层手写多分支 dispatch，改为构造 `SelectionInspectorProviderContext` 并调用 `defaultSelectionInspectorProviderRegistry().drawFirst(...)`。
- provider 内部暂时复用现有 schema facade 与 render helper，因此本轮只改变组织方式，不改变 inspector 字段或 UI 输出。
- `text2.vcxproj` 与 `text2.vcxproj.filters` 已注册新增 provider registry 源文件和头文件。

已完成验证：

- 静态检查确认 `SelectionInspectorProviderRegistry` 已接入 `SelectionInspectorPanel.cpp` 并注册到 Visual Studio 工程。
- 静态检查确认 `drawSelectionInspectorPanel(...)` 不再保留旧的顶层选中目标多分支 `End()+return` dispatch。
- focused verification：`powershell -NoProfile -ExecutionPolicy Bypass -File tools\verify_pbr.ps1 -NoLinkDebugInfo -Modes forward,import,engine-world-editor-create,renderer-backend-registry-noop -DiscardCaptures` 已通过；MSBuild 编译了 `SelectionInspectorPanel.cpp` 与新增 `SelectionInspectorProviderRegistry.cpp`。
- full verification：`powershell -NoProfile -ExecutionPolicy Bypass -File tools\verify_pbr.ps1 -SkipBuild -DiscardCaptures` 已通过，默认 34 个 verification mode 全部通过。
...
### 2026-06-01 PBR Light/Camera Rig Config Schema Adapter

本轮继续 profile/config schema adapter 化，处理 PBR experiment preset 中的 `PBRLightRigProfile` 与 `PBRCameraRigProfile`。目标是让 light/camera rig runtime profile header 不再暴露 editor `PropertyBuilder` 类型，同时保持 `lightRig.*`、`cameraRig.*` 配置 key、apply/copy runtime 行为和 PBR experiment preset 语义不变。

新增与修改：

- 新增 `tools/sceneSetup/PBRLightRigProfileConfig.h/.cpp`，提供 `buildPBRLightRigProfileConfigSchema(...)`，集中生成 Ambient、Directional、Spot 与 Point Lights 配置 schema。
- 新增 `tools/sceneSetup/PBRCameraRigProfileConfig.h/.cpp`，提供 `buildPBRCameraRigProfileConfigSchema(...)`，集中生成 Camera Rig 配置 schema。
- `PBRLightRigProfile.h` 与 `PBRCameraRigProfile.h` 删除 `GL_EDITOR::PropertyBuilder` forward declaration 和 `visitEditableProperties(...)`，runtime profile header 只保留数据、apply/copy 和运行时转换 API。
- `PBRLightRigProfile.cpp` 与 `PBRCameraRigProfile.cpp` 删除 schema 构造实现与 `PropertySchema.h` 直接 include，只保留 light/camera runtime apply/copy 行为。
- `PBRExperimentProfile.cpp` 的 `lightRig.*` 与 `cameraRig.*` 子配置 load/save 改为调用对应 config adapter，保持 preset key 不变。
- `text2.vcxproj` 与 `text2.vcxproj.filters` 注册新增 Light/Camera rig config adapter 源文件和头文件。

已完成验证：

- 静态检查确认 `PBRLightRigProfile::visitEditableProperties`、`PBRCameraRigProfile::visitEditableProperties`、`loadedLightRigProfile.visitEditableProperties(...)`、`loadedCameraRigProfile.visitEditableProperties(...)`、`lightRigSnapshot.visitEditableProperties(...)` 和 `cameraRigSnapshot.visitEditableProperties(...)` 不再存在。
- `git diff --check` 已通过，仅保留当前仓库已有的 LF/CRLF warning。
- focused verification：`powershell -NoProfile -ExecutionPolicy Bypass -File tools\verify_pbr.ps1 -NoLinkDebugInfo -Modes forward,import,texture-set,engine-world-editor-create,renderer-backend-registry-noop -DiscardCaptures` 已通过；MSBuild 编译了 `PBRLightRigProfile.cpp`、`PBRLightRigProfileConfig.cpp`、`PBRCameraRigProfile.cpp`、`PBRCameraRigProfileConfig.cpp`、`PBRExperimentProfile.cpp`、`RuntimePBRLightCameraRigVerification.cpp` 和 `SceneSetup.cpp`。
- full verification：`powershell -NoProfile -ExecutionPolicy Bypass -File tools\verify_pbr.ps1 -SkipBuild -DiscardCaptures` 已通过，默认 34 个 verification mode 全部通过。
